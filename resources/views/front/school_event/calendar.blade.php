...
<div class="row">
    <div class="col-md-12">
        <div class="select-school">
            <div class="nav-top">
                <a href="{{route('schoolevent.index', [$school_id])}}" class="back-top"><img src="{{asset('images/front/arr-left.png')}}">{{translate('戻る')}}</a>
                <h3>{{translate(\Lang::get('schoolevent.title.calendar'))}}</h3>
            </div>
            <div class="calendar">
                <div class="title">{{translate(getSchool()->name)}}</div>
                <div class="calendar-noti">{{translate('イベントの日付を選んでください')}}</div>
                <div id='calendar'></div>
  ...
  $(document).ready(function() {
         $('#calendar').fullCalendar({
                    ...
                    customButtons: {
                       custom_link: {
                         ...
                         click: function(date) {
                           ...
                           var custom_link = "{{route('schoolevent.eventbymonth')}}"
            ...
            events : [
             ...
             {
                title : '{{$schoolEvent->subject}}',
                start : '{{$schoolEvent->scheduled_at}}',
                end : '{{$schoolEvent->deadline_at}}',
                backgroundColor: '{{$schools_color['event_color']}}',
                borderColor: '{{$schools_color['event_color']}}',
                textColor:'#ffffff'
             },
             @endforeach
            ],
            ...
            dayClick: function(date, jsEvent, view) {
                ...
                $(this).attr('style', 'border: 2px solid red !important');
                 ...
                 $.ajax({
                      url: '{{route('schoolevent.usereventclick')}}',
                      type: 'POST',
                      dataType: 'html',
