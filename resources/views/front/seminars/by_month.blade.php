...
<div class="row">
    <div class="col-md-12">
        <div class="select-school">
            <div class="nav-top">
                <a href="{{route('seminar.calendar',[$school_id])}}" class="back-top"><img src="{{asset('images/front/arr-left.png')}}">{{translate('戻る')}}</a>
            ...
            <div class="calendar">
                ...
                <div class="list-week">
                    ...
                    <div class="day-in-week">
                      ...
                      {{--<a href="{{ route('seminar.seminarbyday') }}?date={{$k}}&school={{$school_id}}">--}}
                                    ...
                                    <div class="event">
                                        <div class="time-to">
                                            ...
                                            @if(isset($event['type']) && $event['type'] == 1)
                                                {{translate('終日')}}
                                            @else
                                                {{Carbon\Carbon::parse($event['start_time'])->format('H')}}:
                                                {{Carbon\Carbon::parse($event['start_time'])->locale('ja_JA')->format('i')}}<br>
                                                {{Carbon\Carbon::parse($event['end_time'])->locale('ja_JA')->format('H')}}:
                                                {{Carbon\Carbon::parse($event['end_time'])->locale('ja_JA')->format('i')}}
                                            @endif
                                            </span>
                                        </div>
