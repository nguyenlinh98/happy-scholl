...
<header class="row mb-4 mr-0 ml-0">
    <div class="col-sm-12 col-md-9">
        ...
        <h4 class="font-weight-bolder">お手紙タイトル：{{ $meeting->subject }}</h4>
...
<div class="row mr-0 ml-0">
    <div class="col-sm-12 col-md-9">
        <div class="float-right mb-2">
            既読・未読の有無
            <a href="{{ route('admin.meeting.class', [$meeting, $schoolClass]) }}{{ request()->query('show') === 'read' ? '?show=all' : '?show=read' }}" class="btn {{ request()->query('show') === 'read' ? 'btn-primary' : 'btn-dark' }} btn-sm">既読</a>
            <a href="{{ route('admin.meeting.class', [$meeting, $schoolClass]) }}{{ request()->query('show') === 'unread' ? '?show=all' : '?show=unread' }}" class="btn {{ request()->query('show') === 'unread' ? 'btn-primary' : 'btn-dark' }} btn-sm">未読</a>
        </div>
    </div>
</div>
<div class="row mr-0 ml-0">
