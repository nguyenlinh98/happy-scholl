...
@section('content')
@if(session('action'))
    @includeIf('admin.seminar.action.' . session('action'))
    @endif
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center mb-3 border-bottom">
        ...
        <div class="btn-toolbar mb-2 mb-md-0" id="toolbar">
            <div class="mr-2 toggle--delete-column--off" id="toolbar-table-buttons">
                <a href="{{ route('admin.seminar.create') }}" class="btn  btn-sm btn-secondary" tabindex="0">
                    {{ hsp_action('create') }}
                </a>
            </div>
    ...
    <div class="form-row mb-3">
        <div class="col-2">
            <a href="{{ route('admin.seminar.index') }}?view=reservation" class="btn btn-block btn-lg {{ request()->query('view', 'reservation') === 'reservation' ? 'btn-primary' : 'btn-dark' }} rounded--lg"><span class="h5 font-weight-bold">予約一覧</span></a>
        </div>
        <div class="col-2">
            <a href="{{ route('admin.seminar.index') }}?view=distribution" class="btn btn-block btn-lg {{ request()->query('view') === 'distribution' ? 'btn-primary' : 'btn-dark' }} rounded--lg"><span class="h5 font-weight-bold">配信一覧</span></a>
        </div>
    </div>
    <table class="table datatable" id="messages_table">
        ...
        @foreach($seminars as $seminar)
            <tr>
                <td>{{ $seminar->sender }}</td>
                <td>{{ $seminar->all_class ? '全体' : 'クラス' }}</td>
                <td>{{ $seminar->subject }}</td>
                <td>{{ $seminar->toLocalizeDateTime('scheduled_at') }}</td>
                <td>
                    @if(request()->query('view', 'reservation') === 'reservation')
                        <a href="{{ route('admin.seminar.edit', $seminar) }}" class="btn btn-secondary"><span class="text-white">修正</span></a>
                    @else
                        <a href="{{ route('admin.seminar.show', $seminar) }}" class="btn btn-secondary">修正</a>
                    @endif
                </td>
