...
<header class=" mb-4">
    ...
    @if($letter->class_for_letter !== false)
    <h3 class="font-weight-bolder">送信先：{{$letter->class_for_letter}}</h3>
    @endif
    <h3 class="font-weight-bolder">お手紙タイトル：{{$letter->subject}}</h3>
...
<div class="row">
    <div class="col-sm-12 col-md-9">
        <div class="float-right mb-2">
            既読・未読の有無
            <a href="{{route('admin.letter.students', [$letter])}}{{request()->query('show') === 'read' ? '?show=all' : '?show=read'}}" class="btn {{request()->query('show') === 'read' ? 'btn-primary' : 'btn-dark'}} btn-sm">既読</a>
            <a href="{{route('admin.letter.students', [$letter])}}{{request()->query('show') === 'unread' ? '?show=all' : '?show=unread'}}" class="btn {{request()->query('show') === 'unread' ? 'btn-primary' : 'btn-dark'}} btn-sm">未読</a>
        </div>
    </div>
</div>
<div class="row">
        ...
        <table class="table datatable" id="letter_receivers" data-controller="datatable">
            <thead>
                <tr>
                    ...
                    @if($letter->class_for_letter === false)
                    <h1 class="font-weight-bolder">送信先：{{$letter->class_for_letter}}</h1>
                    @endif
